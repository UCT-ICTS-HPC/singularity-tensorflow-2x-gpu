...
node('p100') {
    ...
    stage('Build') {
       ...
       sh "sudo /usr/bin/singularity build --notest container/$BUILD_NUMBER-tensorflow.sif tensorflow-gpu-build.def"
     }

    stage('Container Cleanup') {
       // Cleaning up unwanted files from the container.
       sh "/usr/bin/singularity cache clean --name container/$BUILD_NUMBER-tensorflow.sif "
     }

    stage('Running Tests') {
       // Execute the %test scriptlet.
       sh "/usr/bin/singularity test --nv container/$BUILD_NUMBER-tensorflow.sif "
     }
